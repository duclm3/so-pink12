...
public class BookPersistenceImpl
	...
	public List<Book> findByAuthor(
		...
		if (list == null) {
			...
			if (author.isEmpty()) {
				sb.append(_FINDER_COLUMN_AUTHOR_AUTHOR_3);
			}
			else {
				bindAuthor = true;

				sb.append(_FINDER_COLUMN_AUTHOR_AUTHOR_2);
			}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(
				entityCacheEnabled, BookImpl.class, primaryKey);
		}
	}
	...
	@Override
	@Reference(
		target = BOOKPersistenceConstants.SERVICE_CONFIGURATION_FILTER,
		unbind = "-"
	)
	...
	static {
		try {
			Class.forName(BOOKPersistenceConstants.class.getName());
		}
